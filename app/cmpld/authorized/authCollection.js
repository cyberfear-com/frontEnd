		...
		render: function () {
			...
			if (this.state.dfd == "solved") {
				...
				if (page == 'mailBox' && app.user.get("profileVersion") > 1) {
					...
					body = React.createElement(MailboxCollection, { pp: this.props.folder, activePage: this.state.folder, changeFodlerId: this.changeFodlerId, folderId: this.state.folder, updateValue: this.updateValue });
				} else if (page == 'settings' && app.user.get("profileVersion") > 1) {
					//	console.log('settings');
					body = React.createElement(SettingsCollection, { rightPanel: this.props.rightPanel, activePage: this.props.activePage });
				} else if (page == 'settings' && app.user.get("profileVersion") == 1 && this.props.activePage == "updateVersion1") {
					//	console.log('update');
					body = React.createElement(SettingsCollection, { rightPanel: this.props.rightPanel, activePage: 'updateVersion1' });
			...
			return React.createElement(
				'div',
				{ className: 'mailBody', onClick: this.handleClick.bind(this, 'resetTimer'), onTouchEnd: this.handleClick.bind(this, 'resetTimer'), onKeyUp: this.handleClick.bind(this, 'resetTimer') },
				React.createElement(
					'div',
					{ className: 'Top' },

		...
		render: function () {
			...
			if(this.state.dfd=="solved"){
				...
				if(page=='mailBox' && app.user.get("profileVersion")>1){
					...
					body=<MailboxCollection pp={this.props.folder} activePage={this.state.folder} changeFodlerId={this.changeFodlerId} folderId={this.state.folder} updateValue={this.updateValue}/>;
				...
				return (
					<div className="mailBody"  onClick={this.handleClick.bind(this, 'resetTimer')} onTouchEnd={this.handleClick.bind(this, 'resetTimer')} onKeyUp={this.handleClick.bind(this, 'resetTimer')}>
						<div className="Top">
							<Header  changeFodlerId={this.changeFodlerId}/></div>
						{body}
						<div className="lds-spinner hidden" id="spinn">
							<div></div>
							<div></div>
							<div></div>
							<div></div>

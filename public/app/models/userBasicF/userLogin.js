...
define(["app"], function (app) {
    var UserLogin = Backbone.Model.extend({
        ...
        Login: function (emailInput, password, factor2, callback) {
            ...
            else var email = emailInput.toLowerCase();
            ...
            var post = {
                ...
                password2step: app.transform.SHA512(
            ...
            app.serverCall.ajaxRequest("loginUser", post, function (result) {
                if (result["response"] == "success") {
                    if (result["data"]["status"] == "welcome") {
                        ...
                        app.user.set({
                            profileVersion: parseInt(
                                result["data"]["userObjectVersion"]
                            ),
                        });
                        ...
                        app.user.set({
                            userLoginToken: result["data"]["token"],
                        });
                        ...
                        app.user.set({
                            cacheEmId:
                                app.transform.SHA256(email).substring(0, 10) +
                                result["data"]["userId"],
                        });
                        app.user.set({
                        ...
                        if (result["data"]["oneStep"]) {
                            ...
                            //console.log(password);
                            app.user.set({
                                secondPassword: app.globalF.makeDerived(
                                    password,
                                    app.transform.hex2bin(
                                        result["data"]["salt"]
                                    )
                                ),
                            });
        ...
        logout: function () {
            app.serverCall.ajaxRequest(
                ...
                {},
                function (result) {
                    if (result["response"] == "success") app.restartApp();
                },
                function () {}
            );
        },
        ...
        checkPlan: function () {
            ...
            if (!app.user.get("getPlan")) {
                ...
                app.userObjects.loadUserPlan(function () {
                    ...
                    //if paid
                    if (
                        app.user.get("userPlan")["newUser"] === false &&
                        ...
                        app.user.get("tempCoin") ||
                        (app.user.get("userPlan")["newUser"] !== true &&
                            app.user.get("firstTimeCacel") === false)
                    ) {
                        //if processing
                        ...
                        app.user.get("userPlan")["newUser"] === true ||
                        app.user.get("firstTimeCacel") === true
                    ) {
                        //if cancelled
